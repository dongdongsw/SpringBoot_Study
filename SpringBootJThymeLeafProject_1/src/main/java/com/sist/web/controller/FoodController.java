 ...
 * 						@Autowired => 스프링에서 자동으로 객체를 찾아서 주소값을 넣어줌
 * 						A a
 ...
 * 														=> Vue/React/모바일
 * 														=> JSON
 * 										@ControllerAdvice : 모든 Controller에서 예외처리를 공통으로 적용
 * 						= AOP : 횡단지향적 프로그램
 * 								=> OOP에서는 불가능한 프로그램을 제공
 * 								=> OOP를 보완
 * 								=> OOP에서는 자동 호출이 불가능
 * 								   ---- 소스 중복이 있는 경우
 ...
 * 										= @After-Returning : 정상 수행 => return이 된 상태
 * 										= 코드
 * 										-------- 1
 * 											코드
 * 										-------- 2 => @Around L 트랜잭션 / 보안 / 로그 파일
 * 									JoinPoint / PointCut => Advice
 * 															-------- 여러개 통합 : Aspect
 * 									=> 클래스의 기능 별 분리가 되어 있다 : 어노테이션 사용
 * 										유지보수가 편리하다
 * 						= MVC
 * 							1. 클라이언트 요청 발생
