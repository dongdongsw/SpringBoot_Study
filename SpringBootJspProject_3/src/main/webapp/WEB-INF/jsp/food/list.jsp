...
<body>
				  ...
				  <div class="col-md-3" v-for="vo in list">
				    <div class="thumbnail">
				      <a :href="'http://localhost:9090/food/detail?fno=' + vo.fno">
				        <img :src="vo.poster" :title="vo.address" alt="Lights" style="width:250px; height: 150px">
				        <div class="caption">
				          <p>{{vo.name}}</p>
				        </div>
				      </a>
	...
	<script>
		let foodApp = Vue.createApp({
			// Model => 데이터 관리 : 데이터가 변경이 되면 HTML 바로 적용
			// created() => Vue 객체 생성 완료
			// mounted() : HTML이 브라우저에 출력이 된경우 :onload
			// $(function){}) => 다른 Front와 연동
			// updated() : 데이터가 수정이 된 경우 : 버튼 / 값 입력 => v-model
			// unmounted() : 화면 변경 => Vue 객 체 소멸
			data(){
				return{
					list:[],
					curpage:1,
