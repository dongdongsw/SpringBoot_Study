 ...
 * 		1) 스프링 개발 / 환경 설정
 * 			= 스프링 프레임워크의 역사 , 철학
 * 							---- 세미나 => 로드 존슨(느슨한 결합) : 오픈소스에 개발
 * 							---- 2.5 : POJO (단독 클래스) : 인터페이스 / 상속 없이 처리
 * 							---- 	   --------------- 관리자 (Spring)
 * 			= 아키텍쳐 구조
 * 				MVC 
 ...
 * 						 새로운 기능이 아니라 설정을 대신해서 하는 프레임 워크
 * 			 # 스프링 프레임 워크보다 => 빠른개발이 가능
 * 			 # 설정을 최소화
 * 			 # 내장 톰캣을 사용 => 서버 설치가 필요 없다
 * 				CI/CD
 ...
 *  				| 전송할 파일 지정
 *  
 *  			=> 매개변수 : 사용자 요청값 / 내장 객체 => 순서는 상관 없다
 *  						커맨드 객체 사용 : VO단위로 값을 받는 경우
 *  						게시판 / 댓글 / 회원가입 => <from>
 *  			@RequestParam => getParameter()
 *  			@ModelAttribute
 *  			@RequestBody ==> JSON = 객체 반환
 ...
 *  				<input type=text name=subject> => VO에는 String name
 *  
 *  			=> 화면 출력
 *  				EL / JSTL
 *  					 ----
 *  					<c:forEach> <c:if> <c:choose> <c:forTokens> <c:set>
 *  
 ...
 *  		객체 생성 / 객체 소멸
 *  		1) XML (framework)
 *  			한개의 객체를 생성 <bean id="", class="">
 *  							생성시에 필요한 데이터 추가
 *  							p:변수 => setter DI
 *  							c:변수 => constructor DI
 *  			패키지별 객체 생성
 *  			<context:component-scan basepackag="">
