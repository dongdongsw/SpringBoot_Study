...
@Mapper // 매핑 => 어노테이션 / xml 연결
@Repository // 메모리할당 = Spring이 관리가 가능하게
/*
	Spring => 클래스 관리 (Container)
				| 의존성이 낮운 프로그램 (수정시에 다른 클래스에 영향이 없는 프로그램)
			어떤 클래스 관리
			VO / Entity
			|		|
			---------
			...
			| @Controller
			| @RestController
			| @ControllerAdvice => 스프링에서 관리하느 클래스
									| 객체 생성 ~ 객체 소멸
			=> 스프링안에서 객체 (싱글턴)를 찾기
				=> @Autowired
				=> 생성자를 통해 객체를 자기
...
public interface SeoulMapper {
	@Select("SELECT no, title, poster, address, msg, rownum "
